...
const Index = (props: CanvasProps & InteractiveMap.DrawProps) => {
  ...
  const [baseMap, baseMapStatus] = useLocalImages(mapData.svgPath);
  ...
  const handleMouseUp = (e: KonvaEventObject<MouseEvent>) => {
    ...
    if ((strokeType === 'draw' || strokeType === 'eraser') && drawTempPoints.length > 0) {
      ...
      setDrawLinesChange?.([...drawLines, data as any]);
    }
    // 初始化
  ...
  // 察觉到切换橡皮擦直接清空所有线条
  useEffect(() => {
    if (strokeType === 'eraser') setDrawLinesChange([]);
  }, [strokeType]);
